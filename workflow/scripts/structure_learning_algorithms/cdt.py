...
def wrapper():
    ...
    elif(snakemake.params["alg"] == "cgnn"):
        from cdt.causality.graph import CGNN
        cgnn = CGNN(nh=int(snakemake.wildcards["nh"]),
                    nruns=int(snakemake.wildcards["nruns"]),
                    batch_size=int(snakemake.wildcards["batch_size"]),
                    lr=float(snakemake.wildcards["lr"]),
                    train_epochs=int(snakemake.wildcards["train_epochs"]),
                    test_epochs=int(snakemake.wildcards["test_epochs"]))
        nx_g = cgnn.create_graph_from_data(df)

    elif(snakemake.params["alg"] == "sam"):
        from cdt.causality.graph import SAM
        sam = SAM()
        nx_g = sam.create_graph_from_data(df)
